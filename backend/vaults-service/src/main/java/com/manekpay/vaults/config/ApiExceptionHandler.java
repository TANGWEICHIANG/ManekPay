...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleGoalNotFound(GoalNotFoundException ex, HttpServletRequest request) {
        return build(HttpStatus.NOT_FOUND, ex.getMessage(), request);
    }

    // Backstop for the unique goal-name-per-customer constraint (V4 migration) - turns the raw
    // insert failure into a sane response instead of an opaque 500.
